...
class SelfAttnBlock(eqx.Module):
    ...
    def __call__(self, inputs, scale=None, shift=None):
...
class Encoder(eqx.Module):
    ...
    def __init__(
        ...
        num_heads: int,
        mlp_ratio: int,
        in_channels: int = 1, # scalar for PDE params
        layer_norm_eps: float = 1e-5,
        pos_emb_init: Callable = get_2d_sincos_pos_embed,
        use_param_film: bool = True,
        film_depth: int = 2,
        film_act: str = "silu",
        **kwargs
    ):
        k_film, *k_blocks = jr.split(key, depth + 1)
        ...
        self.param_film = Mlp(
            ...
            hidden_dim=emb_dim,
            out_dim=depth*emb_dim*2,
            in_dim=in_channels,
            act=film_act
        ) if use_param_film else None
        
        # if use_param_film:
        #     self.param_film = eqx.tree_at(
        #         lambda m: (m.layers[-1].weight, m.layers[-1].bias),
        #         self.param_film,
        #         (jnp.zeros_like(self.param_film.layers[-1].weight), 
        #         jnp.zeros_like(self.param_film.layers[-1].bias))
        #     )
    ...
    def __call__(self, u):
        ...
        out = self.pos_emb
        if self.param_film is not None:
            film = self.param_film(u)  # (depth*emb_dim*2)
            film = film.reshape(self.depth, 2, self.emb_dim)  # (depth, 2, emb_dim)
            scale = film[:, 0, :]  # (depth, emb_dim)
            shift = film[:, 1, :]  # (depth, emb_dim)
        
            for i, block in enumerate(self.blocks):
                out = block(out, scale=scale[i], shift=shift[i])
